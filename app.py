...
from googlesearch import search as google_search_lib # Keep alias for clarity
...
IMAGE_PATTERN = re.compile(r'https://pps\.whatsapp\.net/[^"\s]+') # Used in validate_link
HEADERS = { # Global headers for requests
...
def validate_link(link): # This function remains as per previous enhanced version
    result = {
        "Group Name": "Unknown",
        "Group Link": link,
        "Logo URL": "",
    ...
    try:
        ...
        response.encoding = 'utf-8' # Preserve emojis/Urdu
        ...
        if meta_image and meta_image.get('content'):
            ...
            if IMAGE_PATTERN.match(img_src): # IMAGE_PATTERN defined globally
        ...
        result["Status"] = f"Network Error" # Simplified
    except Exception:
        result["Status"] = f"Parsing Error" # Simplified
    return result

def scrape_whatsapp_links_from_url(url, session):
    """Scrape WhatsApp group links from a webpage.
    Revised to closely match the logic of the user's working simpler scraper."""
    links = set()
    try:
        url_parse_for_error = urlparse(url)
        netloc_for_error = url_parse_for_error.netloc if url_parse_for_error.netloc else url[:30]

        # Use global HEADERS, timeout. Session passed for potential connection pooling.
        # No explicit encoding set on response object, letting requests/BeautifulSoup handle it.
        response = session.get(url, headers=HEADERS, timeout=15) # Slightly increased timeout
        
        soup = BeautifulSoup(response.text, 'html.parser')
        
        # 1. Find links in <a> tags
        for a_tag in soup.find_all('a', href=True):
            href = a_tag.get('href') # Use .get() for safety if href attribute is missing
            if href and href.startswith(WHATSAPP_DOMAIN):
                # Normalize by removing query parameters
                links.add(href.split('?')[0]) 
        
        # 2. Find links in plain text using a regex similar to user's working version
        # User's regex: r'https?://chat\.whatsapp\.com/[^\s]+'
        # This iterates over text segments, which is what user's original code did with soup.stripped_strings
        for text_chunk in soup.stripped_strings:
            if WHATSAPP_DOMAIN in text_chunk: # Pre-filter chunks
                # Apply a regex that's robust like the user's original one.
                # [^\s"\'<>()]+ is a slight safety improvement over just [^\s]+
                # to avoid over-matching into broken HTML or JS in some edge cases.
                found_in_chunk = re.findall(r'https?://chat\.whatsapp\.com/[^\s"\'<>()]+', text_chunk)
                for link_url in found_in_chunk:
                    links.add(link_url.split('?')[0]) # Normalize here as well

    except requests.exceptions.Timeout:
        st.sidebar.warning(f"Timeout scraping {netloc_for_error}", icon="⏱️")
    except requests.exceptions.RequestException as e:
        st.sidebar.warning(f"Scrape error on {netloc_for_error}: {type(e).__name__}", icon="⚠️")
    except Exception as e: # Catch any other parsing errors
        st.sidebar.warning(f"Content parsing error on {netloc_for_error}: {type(e).__name__}", icon="💣")
    
    return list(links)


def perform_google_search(query, num_results_to_fetch, pause_duration):
    """Fetch URLs from Google's search results using googlesearch-python."""
    try:
        st.sidebar.info(f"Googling '{query}' (target: {num_results_to_fetch} results, pause: {pause_duration}s)...")
        # `num_results` is the correct parameter name for the googlesearch library as per user's working code
        ...
        st.error(f"🚫 Google Search Error for '{query}': Parameter issue with the search library ({te}). This might be an internal configuration problem or library version mismatch.")
        return []
    except Exception as e: # General error handling for Google search
    ...
    with st.spinner(f"Crawling {base_domain} (Max Depth: {max_depth}, Max Pages: {max_pages})..."):
        ...
        while urls_to_visit and page_count < max_pages:
            ...
            try:
                ...
                # No explicit encoding set here either
                ...
                if depth < max_depth:
                    ...
                    for link_tag in soup.find_all('a', href=True):
                        ...
                        if parsed_abs_url.scheme in ['http', 'https'] and \
                           ...
                           (abs_url, depth + 1) not in urls_to_visit:
...
def main():
    ...
    with st.sidebar:
        ...
        google_search_pause_seconds = 2.5 # Default
        crawl_depth = 2
        max_crawl_pages = 50
        ...
        if "Google" in input_method:
            ...
            google_search_pause_seconds = st.slider(
                ...
                help="Seconds to pause between Google search requests to avoid rate-limiting."
            )
        if "Entire Website" in input_method:
            st.warning("⚠️ Extensive website crawling can be very slow and resource-intensive.", icon="🚨")
            crawl_depth = st.slider(
                "Max Crawl Depth:", min_value=0, max_value=10, value=2,
                help="How many levels deep to crawl from the start URL."
            )
            max_crawl_pages = st.slider(
                "Max Pages to Crawl:", min_value=1, max_value=1000, value=50,
    ...
    session = requests.Session() # Create session once for multiple requests if needed
    try:
        if input_method == "Search & Scrape from Google (Single Keyword)":
            ...
            excel_file = st.file_uploader("Upload Excel file with keywords", type=["xlsx"])
            ...
            if st.button("🌐 Crawl Website & Validate", use_container_width=True):
                if not domain_url: st.warning("Please enter a domain URL.")
                    ...
                    pages_to_scrape_from, crawl_session = crawl_website(domain_url, max_depth=crawl_depth, max_pages=max_crawl_pages) # crawl_website returns its own session
                    if pages_to_scrape_from:
                        st.info(f"Crawler finished. Now scraping {len(pages_to_scrape_from)} found pages...")
                        ...
                        for i, page_url_to_scrape in enumerate(pages_to_scrape_from):
                            ...
                            links_from_page = scrape_whatsapp_links_from_url(page_url_to_scrape, crawl_session) # Use session from crawl
                            all_scraped_links.update(links_from_page)
                            crawl_scrape_progress.progress((i + 1) / len(pages_to_scrape_from))
                        status_text_crawl_scrape.success("Website scraping complete.")
                    else: st.warning("No pages were found or scraped from the domain based on settings.")
                    crawl_session.close() # Close the session used by the crawler
        ...
        session.close() # Ensure the main session is closed
    ...
    if st.session_state.results:
        ...
        error_statuses = ['Error', 'Timeout Error', 'Network Error', 'Parsing Error', 'Validation System Error']
        ...
        with col3: st.metric("Expired/Invalid", expired_invalid_df_count, help="Links confirmed as expired, revoked, or invalid.")
        ...
        if not active_df.empty:
            ...
            with st.expander("Copy or Download Markdown", expanded=True):
                st.text_area("Markdown Table (Copy this):", value=markdown_table_data, height=250, key="markdown_output_area_v2", help="Select all text (Ctrl+A or Cmd+A) and copy (Ctrl+C or Cmd+C).") # Changed key
                st.download_button("📥 Download Markdown (.md)", markdown_table_data, "active_whatsapp_groups.md", "text/markdown", use_container_width=True, key="download_markdown_button_v2") # Changed key
