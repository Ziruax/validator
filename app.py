...
.whatsapp-groups-table td { padding: 12px; vertical-align: middle; text-align: left; font-size: 0.95em; word-break: break-word;}
/* Specific column styling for styled table */
.whatsapp-groups-table th:nth-child(1), .whatsapp-groups-table td:nth-child(1) { text-align: center; width: 80px; } /* Logo */
.whatsapp-groups-table th:nth-child(3), .whatsapp-groups-table td:nth-child(3) { text-align: right; width: 150px; } /* Join Button */
...
def load_links_from_file(uploaded_file):
    ...
    try:
        ...
        if text_content is None:
            st.error(f"Could not decode file '{uploaded_file.name}'. Please ensure it's UTF-8 or similar.", icon="❌"); return []
...
def validate_link(link):
    ...
    try:
        ...
        if not group_name_str:
            potential_name_tags = soup.find_all(['h2', 'h3', 'strong', 'span', 'div'], class_=re.compile(r'\b(group-name|name|_2v6EX|_1VzZY)\b', re.IGNORECASE)) + \
                                  soup.find_all('div', attrs={'data-testid': 'group-name'})
            for tag in potential_name_tags:
                text = tag.get_text(separator=' ', strip=True)
                if text and 2 < len(text) < 100 and text.lower() not in ["whatsapp group invite", "whatsapp", "join group", "invite link", "open chat", "open this link", "group invite"]:
                    group_name_str = text; break
        result["Group Name"] = group_name_str
        logo_url_str = ""
        for img_tag in soup.find_all('img', src=True): # Renamed variable to avoid conflict
            src = html.unescape(img_tag['src']) # Use renamed variable here
            if src.startswith('https://pps.whatsapp.net/') and IMAGE_PATTERN_PPS.match(src):
                logo_url_str = src; break
        if not logo_url_str:
            meta_image = soup.find('meta', property='og:image')
            if meta_image and meta_image.get('content'):
                src_og = html.unescape(meta_image['content']) # Use different var name
                if OG_IMAGE_PATTERN.match(src_og): logo_url_str = src_og
        result["Logo URL"] = logo_url_str
        page_text_lower = soup.get_text().lower()
        expired_phrases = ["invite link is invalid", "invite link was reset", "group doesn't exist", "this group is no longer available", "couldn't join this group because the invite link is no longer active", "you can't join this group because it's full"]
        is_explicitly_expired_or_full = any(phrase in page_text_lower for phrase in expired_phrases)
        if not group_name_str: result["Status"] = "Inactive"
        elif is_explicitly_expired_or_full:
            result["Status"] = "Full" if "group because it's full" in page_text_lower else "Expired"
        elif logo_url_str: result["Status"] = "Active"
    ...
    try:
        ...
        if WHATSAPP_DOMAIN in text_content:
            ...
            for match in raw_found_links:
                ...
                clean_path = parsed_url.path.split(';')[0].split('?')[0].split('&')[0]
                if len(clean_path.replace('/', '')) >= 16: links.add(f"{parsed_url.scheme}://{parsed_url.netloc}{clean_path}")
...
def google_search_and_scrape(query, top_n=5):
    ...
    try:
        ...
        if not search_page_urls:
            st.warning(f"No Google results for '{query}'. Reasons: no results, Google blocking, or library issue.", icon="🤔"); return []
        ...
        with requests.Session() as scrape_session:
            for i, url_from_google in enumerate(search_page_urls):
                ...
                newly_found_count = sum(1 for link in wa_links_from_page if link.startswith(WHATSAPP_DOMAIN) and all_scraped_wa_links.add(link) is None)
        ...
        stat_txt.success(f"Scraping of Google results complete. Found {len(all_scraped_wa_links)} unique WA links from '{query}'.")
        return list(all_scraped_wa_links)
    except TypeError as e: st.error(f"Google search TypeError: {e}. Check library/parameters or Google blocking.", icon="❌"); return []
    except Exception as e: st.error(f"Unexpected Google search/scrape error for '{query}': {e}. Check connection/library or Google blocking.", icon="❌"); return []
...
def crawl_website(start_url, max_depth=2, max_pages=50):
    ...
    if not start_url.startswith(('http://', 'https://')): start_url = 'https://' + start_url; st.sidebar.warning(f"Prepending 'https://' to: {start_url}", icon="🔗")
    parsed_start_url = urlparse(start_url)
    if not parsed_start_url.netloc: st.sidebar.error(f"Invalid start URL: {start_url}. Please include a domain.", icon="🚫"); return scraped_whatsapp_links
    ...
    with requests.Session() as session, st.spinner(f"Crawling {base_domain}... (Depth: {max_depth}, Pages: {max_pages})"):
        while queue_list and page_count < max_pages:
            if len(queue_list) > max_q_size: st.sidebar.warning(f"Crawl queue > {max_q_size}. Stopping early.", icon="❗️"); queue_list = queue_list[:max_q_size]
            current_url, depth = queue_list.pop(0)
            normalized_current_url_for_check = urljoin(current_url, urlparse(current_url).path or '/')
            # if normalized_current_url_for_check in visited_normalized_urls and current_url != start_url: continue # Already handled by check before adding
            ...
            try:
                ...
                newly_found_count = sum(1 for link in wa_links_from_page if link.startswith(WHATSAPP_DOMAIN) and scraped_whatsapp_links.add(link) is None)
                        ...
                        if href:
                            ...
                            if parsed_abs_url.scheme in ['http', 'https'] and parsed_abs_url.netloc.replace('www.', '') == base_domain:
                                ...
                                if normalized_abs_url_for_visited not in visited_normalized_urls and abs_url not in urls_in_queue_full:
                                    if len(queue_list) < max_q_size: queue_list.append((abs_url, depth + 1)); urls_in_queue_full.add(abs_url)
                                    else: st.sidebar.warning("Crawl queue full, skipping.", icon="🟡"); break 
            ...
            time.sleep(0.2) 
    st.sidebar.success(f"Crawl complete. Scraped {page_count} pages, found {len(scraped_whatsapp_links)} links.")
...
def generate_styled_html_table(data_df_for_table):
    if data_df_for_table.empty: return "<p style='text-align:center; color:#777; margin-top:20px;'><i>No 'Active' groups match display filters.</i></p>"
    html_string = '<table class="whatsapp-groups-table" aria-label="List of Active WhatsApp Groups"><caption>Filtered Active WhatsApp Groups</caption><thead><tr><th scope="col">Logo</th><th scope="col">Group Name</th><th scope="col">Group Link</th></tr></thead><tbody>'
    for _, row in data_df_for_table.iterrows():
        ...
        html_string += f'</td><td class="group-name-cell">{html.escape(group_name)}</td><td class="join-button-cell">'
        if group_link and group_link.startswith(WHATSAPP_DOMAIN): html_string += f'<a href="{html.escape(group_link)}" class="join-button" target="_blank" rel="noopener noreferrer">Join Group</a>'
...
def main():
    ...
    # MODIFIED: Default Filter - adv_filter_status will be set later if conditions met
    ...
    if 'adv_filter_status_default_applied_once' not in st.session_state: st.session_state.adv_filter_status_default_applied_once = False

    # Populate processed_links_in_session from existing results
    ...
    with st.sidebar:
        ...
        input_method = st.selectbox("Choose Input Method:", ["Search and Scrape from Google", "Search & Scrape from Google (Bulk via Excel)", "Scrape from Specific Webpage URL", "Scrape from Entire Website (Extensive Crawl)", "Enter Links Manually (for Validation)", "Upload Link File (TXT/CSV/Excel)"], key="input_method_main_select", index=0)
        gs_top_n = 5 
        if input_method in ["Search and Scrape from Google", "Search & Scrape from Google (Bulk via Excel)", "Upload Link File (TXT/CSV/Excel)"]:
            gs_top_n = st.slider("Google Results to Scrape (per keyword)", 1, 100, 5, key="gs_top_n_slider", help="Higher values (>20) may be slow or lead to Google blocks.")
        ...
        if st.button("🗑️ Clear All Results & Reset Filters", use_container_width=True, key="clear_all_button"):
            ...
            st.session_state.styled_table_name_keywords, st.session_state.adv_filter_name_keywords = "", ""
            st.session_state.styled_table_current_limit_value = 50
            st.session_state.adv_filter_status = [] # MODIFIED: Reset to empty for re-evaluation of default
            st.session_state.adv_filter_status_default_applied_once = False # MODIFIED: Allow default to be reapplied
    ...
    # ... (Input method handling code - kept concise for brevity, assumed unchanged from previous correct version)
    try:
    ...
    if links_to_validate_now:
        st.success(f"Total {len(current_action_scraped_links)} unique links gathered. Validating {len(links_to_validate_now)} new links...")
        ...
        with ThreadPoolExecutor(max_workers=MAX_VALIDATION_WORKERS) as executor:
            ...
            for i, future_item in enumerate(as_completed(future_to_link_map)):
                ...
                try:
                    ...
                    parsed_url_val_item = urlparse(link_validated_item); normalized_link_val_item = f"{parsed_url_val_item.scheme}://{parsed_url_val_item.netloc}{parsed_url_val_item.path}"
                    ...
                    parsed_url_val_err_item = urlparse(link_validated_item); normalized_link_val_err_item = f"{parsed_url_val_err_item.scheme}://{parsed_url_val_err_item.netloc}{parsed_url_val_err_item.path}"
                    st.session_state.processed_links_in_session.add(normalized_link_val_err_item)
                    new_results_this_run.append({"Group Name": "Validation Error", "Group Link": link_validated_item, "Logo URL": "", "Status": f"CritValidationFail: {type(val_exc_item).__name__}"})
                prog_val.progress((i+1)/len(links_to_validate_now)); status_update_text = f"Validated {i+1}/{len(links_to_validate_now)}. Last: ...{link_validated_item[-25:]}, Status: {new_results_this_run[-1]['Status']}"; stat_val.text(status_update_text)
        ...
        st.info("All WhatsApp links from this action were already processed. No new links to validate.")
    ...
    if 'results' in st.session_state and st.session_state.results:
        ...
        # MODIFIED: Accurate Results Summary Calculation
        ...
        expired_full_df_master = df_display_master_final[df_display_master_final['Status'].isin(['Expired', 'Full'])].copy()
        error_other_df_master = df_display_master_final[~df_display_master_final['Status'].isin(['Active', 'Inactive', 'Expired', 'Full'])].copy()
        ...
        summary_col4.markdown(f'<div class="metric-card">Expired/Full/Error<br><div class="metric-value">{len(expired_full_df_master) + len(error_other_df_master)}</div></div>', unsafe_allow_html=True)

        # MODIFIED: Collapsible Result Sections
        st.subheader("📋 Detailed Results by Category")

        with st.expander(f"⭐ Active Groups ({len(active_df_master)}) - Click to Expand", expanded=True): # Active expanded by default
            if not active_df_master.empty:
                ...
                with st.form("styled_table_filters_form_active"):
                    name_kw_styled = st.text_input("Filter by Group Name Keywords (comma-separated):", value=st.session_state.styled_table_name_keywords, placeholder="e.g., study, fun, tech", help="Shows active groups matching ANY keyword.").strip()
                    limit_styled = st.number_input("Max Active Groups to Display:", 1, max(1, len(active_df_master)), st.session_state.styled_table_current_limit_value, 10, help="Max active groups for styled table.")
                ...
                if st.button("Reset Styled Table Filters", key="reset_styled_table_filters_btn_active"):
                ...
                else: st.write("No active groups match current filters.")
                html_table_output = generate_styled_html_table(active_df_styled_final)
                st.markdown(html_table_output, unsafe_allow_html=True)
                st.markdown("---"); st.text_area("Copy Raw HTML Code (for above table):", value=html_table_output, height=150, key="styled_html_export_key_active", help="Ctrl+A, Ctrl+C")
                st.markdown('</div>', unsafe_allow_html=True) 
            else: st.info("No 'Active' groups found in the results yet.")

        with st.expander(f"💨 Expired & Full Groups ({len(expired_full_df_master)}) - Click to Expand", expanded=False):
            if not expired_full_df_master.empty:
                st.dataframe(expired_full_df_master, use_container_width=True, hide_index=True,
                             column_config={"Group Link": st.column_config.LinkColumn(display_text="Link"), "Logo URL": st.column_config.LinkColumn(display_text="Logo")})
            else: st.info("No 'Expired' or 'Full' groups found.")
            
        with st.expander(f"😶 Inactive (Nameless) Groups ({len(inactive_df_master)}) - Click to Expand", expanded=False):
            if not inactive_df_master.empty:
                st.dataframe(inactive_df_master, use_container_width=True, hide_index=True,
                             column_config={"Group Link": st.column_config.LinkColumn(display_text="Link"), "Logo URL": st.column_config.LinkColumn(display_text="Logo")})
            else: st.info("No 'Inactive' (nameless) groups found.")

        with st.expander(f"⚠️ Other Statuses (Errors, Redirects, etc. - {len(error_other_df_master)}) - Click to Expand", expanded=False):
            if not error_other_df_master.empty:
                st.dataframe(error_other_df_master, use_container_width=True, hide_index=True,
                             column_config={"Group Link": st.column_config.LinkColumn(display_text="Link"), "Logo URL": st.column_config.LinkColumn(display_text="Logo")})
            else: st.info("No groups with 'Other' statuses (like errors, redirects) found.")

        # Advanced Filtering for Downloads (remains for CSV generation)
        with st.expander("🔬 Advanced Filtering for 'All Processed Results' CSV", expanded=False):
            st.markdown('<div class="filter-container" style="border-style:solid;">', unsafe_allow_html=True)
            st.markdown("#### Filter Dataset for CSV Download (Excludes 'Inactive' by Default):")
            ...
            # MODIFIED: Default Active Filter for this section
            if not st.session_state.adv_filter_status_default_applied_once and \
               not st.session_state.adv_filter_status and \
               "Active" in all_statuses_adv:
                st.session_state.adv_filter_status = ["Active"]
                st.session_state.adv_filter_status_default_applied_once = True 
            
            adv_status_sel = st.multiselect("Filter by Status:", options=all_statuses_adv, default=st.session_state.adv_filter_status, key="adv_status_filter_ms_key_dl", help="Select statuses. 'Inactive' groups are already excluded.")
            if adv_status_sel != st.session_state.adv_filter_status:
                st.session_state.adv_filter_status = adv_status_sel; st.session_state.adv_filter_status_default_applied_once = True 
            adv_name_kw_sel = st.text_input("Filter by Group Name Keywords (comma-separated):", value=st.session_state.adv_filter_name_keywords, key="adv_name_kw_filter_input_key_dl_csv", placeholder="e.g., news, jobs").strip()
            if adv_name_kw_sel != st.session_state.adv_filter_name_keywords: st.session_state.adv_filter_name_keywords = adv_name_kw_sel
            st.markdown('</div>', unsafe_allow_html=True)

            df_for_adv_dl_view = df_base_adv_dl.copy(); adv_filters_applied_dl_flag = False
            if st.session_state.adv_filter_status:
                df_for_adv_dl_view = df_for_adv_dl_view[df_for_adv_dl_view['Status'].isin(st.session_state.adv_filter_status)]; adv_filters_applied_dl_flag = True
            if st.session_state.adv_filter_name_keywords:
                ...
                if adv_kw_list_dl:
                    ...
                    df_for_adv_dl_view = df_for_adv_dl_view[df_for_adv_dl_view['Group Name'].str.lower().str.contains(adv_regex_pat_dl, na=False, regex=True)]; adv_filters_applied_dl_flag = True
            
            preview_lbl_dl = "All (Excl. Inactive)" if not adv_filters_applied_dl_flag else "Filtered (Excl. Inactive)"
            st.markdown(f"**Preview of Data for CSV ({preview_lbl_dl} - {len(df_for_adv_dl_view)} rows):**")
            st.dataframe(df_for_adv_dl_view, column_config={"Group Link": st.column_config.LinkColumn("Invite Link"), "Logo URL": st.column_config.LinkColumn("Logo")}, hide_index=True, height=200, use_container_width=True)

        st.subheader("📥 Download Results (CSV)")
        dl_col_main1, dl_col_main2 = st.columns(2)
        if not active_df_master.empty:
            dl_col_main1.download_button(label=f"Active Groups ({len(active_df_master)} rows) (CSV)", data=active_df_master.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig'), file_name="active_whatsapp_groups.csv", mime="text/csv", use_container_width=True, key="dl_active_csv_btn_key_main")
        else: dl_col_main1.button("Active Groups (CSV)", disabled=True, use_container_width=True, help="No 'Active' groups.")
        if not df_for_adv_dl_view.empty:
            dl_lbl_all_proc = f"Processed (Excl. Inactive{' - Filtered' if adv_filters_applied_dl_flag else ''}) ({len(df_for_adv_dl_view)} rows)"
            dl_col_main2.download_button(label=f"{dl_lbl_all_proc} (CSV)", data=df_for_adv_dl_view.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig'), file_name="processed_whatsapp_groups.csv", mime="text/csv", use_container_width=True, key="dl_all_filt_csv_btn_key_main")
        elif not df_base_adv_dl.empty() and df_for_adv_dl_view.empty() and adv_filters_applied_dl_flag: dl_col_main2.button("No Results Match Adv. Filters (Processed CSV)", disabled=True, use_container_width=True)
        else: dl_col_main2.button("Processed Results (CSV)", disabled=True, use_container_width=True, help="No results (excl. Inactive).")
