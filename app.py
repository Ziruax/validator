...
​
 a−zA−Z]2,4)
', '', clean_link) if not clean_link.endswith(('.html', '.htm', '.php')) else clean_link
...
except requests.exceptions.RequestException as e: st.sidebar.warning(f"Scrape Net Err ({type(e).name}): {url[:50]}...", icon="⚠️")
except Exception as e: st.sidebar.warning(f"Scrape Parse Err ({type(e).name}): {url[:50]}...", icon="💣")
return list(links)
