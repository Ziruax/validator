...
.markdown-table img.group-logo-markdown {
    width: 40px;
    height: 40px;
    border-radius: 50%;
    object-fit: cover;
...
def scrape_whatsapp_links_user_original(url):
    ...
    try:
        headers = {
            "User-Agent": ua_general.random  # Updated to random User-Agent
        }
...
def generate_markdown_output(active_results_df):
    ...
    markdown_lines = ["<table class='markdown-table'>", "<tr><th>Group Logo</th><th>Group Name</th><th>Group Link</th></tr>"]
    for _, row in active_results_df.iterrows():
        ...
        link_md = f'<a href="{group_link}">Join Group</a>'
        safe_group_name = group_name.replace("|", "|")
        markdown_lines.append(f"<tr><td>{logo_md}</td><td>{safe_group_name}</td><td>{link_md}</td></tr>")
    ...
    with st.sidebar:
        ...
        if input_method in ["Search and Scrape from Google", "Search & Scrape from Google (Bulk via Excel)"]:
            google_results_slider_top_n = st.slider(
                ...
                min_value=1, max_value=10, value=5, key="google_top_n_slider"
            )
